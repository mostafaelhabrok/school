    ...
    <div class="row container-fluid">
        <div id="container2" class="col-9 float-left">
            <nav class="nav nav-pills">
              ...
              <a class="nav-link active" href="students.php" target="_self" id="students"hidden>Students</a>
...
<table border="1px" class="table table-bordered table-hover">
  ...
  <tbody id="tablebody">
    ...
    $email="";$grade="";$subject="";$type=type($db_connection);
    
    //$qyery = "select * from accounts where logged='yes';";
   // $result = mysqli_query($db_connection,$qyery);
    //while($row = mysqli_fetch_array($result)) {$email=$row["email"];}
    if($_COOKIE["email"]!=""){
    ...
    $result = mysqli_query($db_connection,$qyery);
    while($row = mysqli_fetch_array($result)) {if($email==$row["email"]){$grade=$row["grade"];}}
    $qyery = "select * from students where grade='$grade';";
    $result = mysqli_query($db_connection,$qyery);
    ...
    while($row = mysqli_fetch_array($result))
    {
      if(strpos($row["lesson"], $subject) !== false){
        echo "<tr>
        <td>{$row['id']}</td>
        <td>{$row['firstname']} {$row['middlename']} {$row['lastname']}</td>
        <td>{$row['gender']}</td>
