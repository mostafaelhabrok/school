...
// if everything is ok, try to upload file
  ...
  if (move_uploaded_file($_FILES["photo"]["tmp_name"], $target_file)) {
   ...
   if($type=="student"){
    ...
    // while($row = mysqli_fetch_array($result)) {
       ...
       //  $pass = $row["password"];
      // if($email==$row["email"]){
         $qyery = "UPDATE students SET firstname = '$fname' , middlename = '$mname' , 
         lastname = '$lname', country = '$country', lesson ='$lessons', grade = '$grade', 
         phone = '$phone', birthday = '$date', gender = '$gender' where email = '$email';";
         $result = mysqli_query($db_connection,$qyery);
 
         echo '<script>alert("profile edited successfully")</script>';
     
         if (!$result) {die("unable to edit data into table students ".mysqli_error($db_connection)); }
    //   break;
   //    }
  ...
  //   }
   ...
   //  while($row = mysqli_fetch_array($result)) {
       ...
       //  $pass = $row["password"];
    //   if($email==$row["email"]){
        $qyery = "UPDATE teachers SET firstname = '$fname' , middlename = '$mname' , 
        lastname = '$lname', country = '$country', grades ='$grades', subject = '$subject', 
        phone = '$phone', birthday = '$date', gender = '$gender' where email = '$email';";
                 $result = mysqli_query($db_connection,$qyery);
 
         echo '<script>alert("profile edited successfully")</script>';
     
         if (!$result) {die("unable to edit data into table teachers ".mysqli_error($db_connection)); }
     //  break;
     //  }
